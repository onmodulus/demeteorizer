 ...
 * @param {String} options.dockerfile Optional Dockerfile to add to the output directory
...
Demeteorizer.prototype.createDockerfile = function (context, callback) {
  ...
  if (!context.options.dockerfile) {
    return callback();
  }

  this.emit('progress', 'Copying Dockerfile.');

  var dockerFile = context.options.dockerfile;

  fs.writeFileSync(context.paths.dockerfile, fs.readFileSync(dockerFile));

  this.emit('progress', 'Dockerfile copied.');
  callback();
};
