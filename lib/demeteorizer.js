 ...
 * root command to use for building (meteor, mrt, etc).
 * @param {Object} context Context of this convert.
...
Demeteorizer.prototype.getBundleCommand = function (context) {
  ...
  try {
    if (semver.lte(context.meteorVersion.replace('x', '0'), '0.8.0')
      ...
      this.emit(
        'progress',
        'Found smart.json file, switching to Meteor build.');
      cmd = 'mrt';
    }
  } catch (e) {
...
Demeteorizer.prototype.bundle = function (context, callback) {
  ...
  this.emit('progress', 'Building Meteor app...');

  if (context.options.debug) {
    this.emit('progress', 'Running Meteor build in debug mode.');
  }

  // meteor bundle --debug --version 0.x.x --directory ./
  var bundleCommand = util.format('cd %s && %s build %s %s --directory %s',
  ...
  childProcess.exec(bundleCommand, function (err, stdout, stderr) {
    ...
    this.emit('progress', cmd + ' build complete.');
    callback();
  }.bind(this));
};
...
Demeteorizer.prototype.filterDep = function (name, version) {
  ...
  //
  // Force the following packages to working versions.
  //
  if (name === 'esprima') {
    filtered.version = '~2.3.0';
  }

  if (name === 'evented') {
    filtered.version = '0.1.0';
  }

  if (['recast', 'regenerator'].indexOf(name) >= 0) {
    filtered.version = '0.x.x';
  }
...
Demeteorizer.prototype.createPackageJSON = function (context, callback) {
  ...
  var arbJson = context.options.arbJson;
  var arbJsonObj = JSON.parse(arbJson);
