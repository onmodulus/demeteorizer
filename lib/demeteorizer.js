...
Demeteorizer.prototype.convert = function(input, output, nodeVersion, release, tarball, appName, prerelease, debug, callback) {
  ...
  self.paths = {
    ...
    server_dir: path.join(output, 'programs', 'server'),
    deps_package: path.join(output, 'programs', 'server', 'package.json'),
  ...
  async.series([
    ...
    function(callback) {
      self.bundle(input, self.paths.bundle, release, prerelease, debug, callback);
    },
    function(callback) {
...
//--------------------------------------------------------------------------------------------------
Demeteorizer.prototype.bundle = function(input, bundle, release, prerelease, debug, callback) {
  ...
  if(debug) {
    debugFlag = ' --debug';
    self.emit('progress', 'Running in Debug mode.');
  }
  ...
  exec('cd ' + escapePath(input) + ' && ' + cmd + ' bundle' + debugFlag + releaseFlag + escapePath(path.basename(bundle)), function(err, stdout, stderr) {
...
Demeteorizer.prototype.findDependencies = function(output, callback) {
  ...
  if(fs.existsSync(this.paths.old_node_modules)) {
    ...
    finishDeps();
  } else if (fs.existsSync(this.paths.node_modules)) {
    nodeModulesDirs.push(this.paths.node_modules);
    finishDeps();
  } else {
     // Meteor 0.9.0 bundle step does not generate node_modules directory
    // a package.json file is there instead, so run npm install to get deps
    // then push to nodeModulesDirs as usual
    self.emit("progress", "No node_modules directory, checking for package.json");
    if (fs.existsSync(this.paths.deps_package)) {
    self.emit("progress", "Running npm install...");

      exec('cd ' + this.paths.server_dir + ';npm install', function(err, stdout, stderr) {

      self.emit('progress', 'npm install finished.');
      
      nodeModulesDirs.push(self.paths.node_modules);
  ...
  self.nodeModuleDirs = nodeModulesDirs;
  self.dependencies = dependencies;

  self.emit('progress', count + ' dependencies found.');
...
Demeteorizer.prototype.setNodeVersion = function(serverFile, nodeVersion, callback) {
  ...
  // Meteor 0.9.0 bundle step makes user's boot.js read-only
  // is this step necessary?
  // handle the error appropriately...writeFileSync doesnt return anything
  try {
    fs.writeFileSync(serverFile, contents);
  } catch (ex) {
    callback("Couldn't write min Node version to " + serverFile + ". Try running again as sudo.");
  }
